...
test('사진과 같은 5개 글꼴, 실행 취소와 다시 실행, 캔버스 배율이 동작한다',async({page})=>{
  await create(page)
  const fonts=page.locator('.font-options')
  await expect(fonts.getByRole('radio')).toHaveCount(5)
  await fonts.getByText('Bebas Neue',{exact:false}).click()
  await expect(page.locator('.preview-panel .midnight').first()).toHaveCSS('font-family',/Bebas Neue/)
...
test('통합 이미지 업로드와 캔버스 이미지 추가가 동작한다',async({page})=>{
  ...
  await page.locator('.preview-host input[type=file]').setInputFiles({name:'canvas.png',mimeType:'image/png',buffer:pixel})
  await expect(page.locator('.preview-panel .overlay-image.interactive')).toBeVisible()
  await expect(page.getByText('이미지 추가 완료',{exact:true})).toBeVisible()
})
