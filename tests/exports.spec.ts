...
function expectRenderedPixels(image:PNG){
  let visible=0
  let nonBlack=0
  let bright=0
  for(let offset=0;offset<image.data.length;offset+=4){
    ...
    visible++
    const total=image.data[offset]+image.data[offset+1]+image.data[offset+2]
    if(total>24)nonBlack++
    if(total>600)bright++
  }
  expect(visible).toBeGreaterThan(0)
  expect(nonBlack/visible).toBeGreaterThan(.8)
  expect(bright).toBeGreaterThan(100)
}
test('PNG 1080x1350, 실제 픽셀, 로컬 글꼴, 떠있는 이미지, ZIP 페이지 순서',async({page})=>{await create(page);await expect.poll(()=>page.evaluate(()=>document.fonts.check('62px Pretendard'))).toBe(true);await expect(page.locator('.midnight blockquote').first()).toHaveCSS('font-family',/Pretendard/);await page.locator('.preview-image-input').setInputFiles({name:'export-overlay.png',mimeType:'image/png',buffer:pixel});const overlay=page.locator('.overlay-image.interactive');await overlay.focus();for(let index=0;index<10;index++){await page.keyboard.press('Shift+ArrowRight');await page.keyboard.press('Shift+ArrowDown')}await expect(overlay).toBeVisible();const pngEvent=page.waitForEvent('download');await page.getByRole('button',{name:'PNG',exact:true}).click();const png=await pngEvent;const pngBuffer=await (await import('node:fs/promises')).readFile(await png.path() as string);const dimensions=PNG.sync.read(pngBuffer);expect(dimensions.width).toBe(1080);expect(dimensions.height).toBe(1350);expectRenderedPixels(dimensions);await page.getByRole('button',{name:'페이지 추가'}).click();const zipEvent=page.waitForEvent('download');await page.getByRole('button',{name:'ZIP',exact:true}).click();const download=await zipEvent;const zipBuffer=await (await import('node:fs/promises')).readFile(await download.path() as string);const zip=await JSZip.loadAsync(zipBuffer);const names=Object.keys(zip.files);expect(names).toHaveLength(2);expect(names[0]).toMatch(/-001\.png$/);expect(names[1]).toMatch(/-002\.png$/);for(const name of names){const image=PNG.sync.read(await zip.file(name)!.async('nodebuffer'));expect([image.width,image.height]).toEqual([1080,1350]);expectRenderedPixels(image)}})
