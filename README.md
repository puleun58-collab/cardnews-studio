...
- 프로젝트와 페이지 생성, 복제, 삭제 및 8초 삭제 실행 취소
...
- 페이지 구조·이미지·템플릿 변경을 포함한 프로젝트별 20단계 실행 취소·다시 실행
- 프로젝트별 마지막 편집 위치 복원과 안전한 템플릿 교체 안내
- 50%, 75%, 100%, 화면 맞춤 미리보기
...
- 페이지 구조 변경, JSON 가져오기와 Undo·Redo는 즉시 저장 큐에 반영합니다.
- 삭제 항목은 8초 동안 `meta` 복구 저널에 보관하며, 만료된 뒤에만 데이터와 고아 이미지 Blob을 정리합니다.
...
- 프로젝트 삭제가 확정되면 연결 페이지, 편집 세션과 사용하지 않는 이미지 Blob을 정리합니다.
- 같은 이미지 Blob은 SHA-256 해시 ID를 사용해 한 번만 저장합니다.

## 편집 안전성

- 프로젝트·페이지 삭제는 8초 안에 원래 위치로 복구할 수 있으며, 새로고침 후에도 남은 복구 작업을 이어갑니다.
- 마지막 페이지와 화면 상태는 프로젝트별로 저장하고, 삭제되었거나 유효하지 않은 페이지는 첫 페이지로 복구합니다.
- 페이지 구조·캔버스·이미지·템플릿 변경은 Undo·Redo에 포함하며, 템플릿 교체 전 잘림·제외 등 데이터 손실 가능성을 안내합니다.
...
- `test:unit`: 저장·마이그레이션, 삭제 복구, 프로젝트별 세션·히스토리, 템플릿 손실 분석과 저장 경쟁을 검사합니다.
- `verify:ui`: 삭제 Undo, 구조 변경 Undo·Redo, 템플릿 교체, 마지막 편집 위치와 모바일·키보드 흐름을 검사합니다.
