...
- 문장 카드의 배경색 또는 배경 이미지 위에 범위와 강도를 조절하는 그라데이션 적용
...
`9장 인사이트 스토리` 구성은 표지에서 시작해 문장, 핵심 수치, 목록, 과정, 비교, 인용·해설, 이미지 설명, 마무리 순서로 생성됩니다. 이전 프로젝트의 `기본 인용` 템플릿은 JSON과 저장 데이터 호환을 위해 계속 렌더링되지만 신규 선택 목록에서는 제외됩니다.
