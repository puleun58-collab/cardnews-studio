...
기존 schemaVersion 1 JSON은 계속 가져올 수 있습니다. 새 백업은 schemaVersion 2로 저장되며, 현재 앱보다 높은 버전은 원본을 변경하지 않고 거부합니다. 브라우저 저장소 초기화는 홈 하단 `브라우저 저장소 관리`에서 할 수 있으며, 이미지 포함 전체 백업 권장 안내와 두 번의 확인 후에만 실행됩니다.
