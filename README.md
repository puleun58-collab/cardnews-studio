...
- 텍스트 편집과 클릭·드래그앤드롭 방식의 통합 이미지 업로드
- 사진형 템플릿의 사진 교체와 모든 템플릿의 떠있는 이미지 위치·크기 조절
...
새 프로젝트는 하나의 템플릿 선택 흐름으로 시작합니다. 편집 화면의 `이미지 업로드` 영역이나 캔버스에 이미지를 놓으면 사진형 템플릿에서는 사진 영역에, 다른 템플릿에서는 이동 가능한 이미지로 추가됩니다. `떠있는 이미지` 설정에서는 별도의 이미지 레이어를 추가하고 위치와 크기를 조절할 수 있습니다.
...
- `verify:ui`: 프로젝트 생성, 템플릿, 통합 이미지 업로드, 글꼴 선택 스크롤 유지, 모바일 화면과 자동 저장을 검사합니다.
