...
- 전체 JSON 백업: 이미지를 포함하며 단일 프로젝트 또는 전체 작업공간을 복원할 수 있음
- 경량 JSON 백업: 프로젝트 구조와 텍스트·디자인만 포함하고 배경·콘텐츠·오버레이 이미지는 제외
- JSON 가져오기 상한: 전체 백업을 고려해 100MB

기존 schemaVersion 1 JSON은 계속 가져올 수 있습니다. 새 백업은 schemaVersion 2로 저장되며, 현재 앱보다 높은 버전은 원본을 변경하지 않고 거부합니다. 브라우저 저장소 초기화는 홈 하단 `브라우저 저장소 관리`에서 할 수 있으며, 전체 JSON 백업 권장 안내와 두 번의 확인 후에만 실행됩니다.
