...
export function StorageStatusPanel({ showMigrationNotice = true }: { showMigrationNotice?: boolean }) {
  ...
  const migrationNotice = showMigrationNotice && !migrationDismissed ? store.migrationNotice : null
  if (!failureMessage && !isNearQuota && !migrationNotice) return null

  const backup = () => {
    const legacy = localStorage.getItem(appConfig.storageKey)
    if (migrationNotice && store.storageError && legacy) {
  ...
  return (
    <aside className={`storage-warning ${failureMessage ? 'is-error' : ''}`} role={failureMessage ? 'alert' : 'status'}>
      <div>
        <strong>{failureMessage ? '자동 저장을 확인해 주세요' : isNearQuota ? '저장 공간이 거의 찼습니다' : '데이터 이전 완료'}</strong>
        <p>{failureMessage ?? migrationNotice ?? '브라우저 저장 공간을 정리해 주세요.'}</p>
      ...
      <div className="storage-warning-actions">
        ...
        <button type="button" className="storage-backup-action" onClick={backup}>이미지 포함 백업</button>
        {(failureMessage || isNearQuota) && <button type="button" onClick={() => void cleanup()}>사용하지 않는 이미지 정리</button>}
        {migrationNotice && !failureMessage && !isNearQuota && (
          <button type="button" className="storage-dismiss-action" onClick={() => setMigrationDismissed(true)}>확인</button>
        )}
      </div>
    </aside>
  )
