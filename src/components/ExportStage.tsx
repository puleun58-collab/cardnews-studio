...
export const ExportStage=forwardRef<ExportHandle>(function ExportStage(_,ref){
  ...
  useImperativeHandle(ref,()=>({async capturePage(page,index,count,size){
    ...
    const node=stage?.querySelector<HTMLElement>('.card-root')
    if(!stage||!node)throw new Error('내보내기 화면을 준비하지 못했습니다.')
    await awaitImagesReady(stage);await waitForFrame();await waitForFrame()
